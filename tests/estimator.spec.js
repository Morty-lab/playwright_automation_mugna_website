    ...
    for(const {team} of projects){
        for (const [key, details]  of Object.entries(team)){
            try {
                ...
                const staffInput = page.locator(`div:nth-child(${roleSelector}) > .ml-auto > .relative > #quantity-input`).first();
                ...
                console.log(`Test passed for role: ${key} with count: ${details.quantity}`);
            } catch (error) {
                console.error(`Test failed for role: ${key} with count: ${details.quantity}`, error);
            }
        }
    }
  });
