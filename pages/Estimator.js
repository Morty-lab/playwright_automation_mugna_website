...
export default class Estimator {
    ...
    async add_staff(buttonType, role,count) {
        ...
        let roleSelector = roleDictionary[role]; // returns the corresponding numeric value for the role
        let buttonSelector;
        switch(buttonType) {
            case 'increment':
                if(roleSelector < 5) {
                    ...
                    buttonSelector = `div:nth-child(2) > div:nth-child(${roleSelector - 4}) > .ml-auto > .relative > #increment-button`;
                }
                break;
            case 'decrement':
                if(roleSelector < 5) {
                    ...
                    buttonSelector = `div:nth-child(2) > div:nth-child(${roleSelector - 4}) > .ml-auto > .relative > #decrement-button`;
                }
                break;
            default:
    ...
    async input_staff(projects){
        for (const { team,months,expected } of projects) {
            ...
            await this.skip()
            await this.page.getByRole('textbox', { name: 'Juan@gmail.com' }).fill('test@mail.com');
            ...
            const textValue = await this.page.locator('.ml-auto.text-right').locator('p').nth(1).innerText();
